...
def generate_idealized_statistics(is_coincident):
    for time_combo, time_range in time_combinations.items():
        for stat_label in statistics.keys():
            for flare_label in flare_labels:
                flare_df = pd.read_csv(f"{idealized_flares}/{is_coincident}/{flare_label}_idealized_flare_{is_coincident}.csv")
                flare_df.drop("Unnamed: 0", inplace=True, axis=1)
                flare_df = flare_df.iloc[time_range]
        ...
        for stat_label, stat_df in statistics.items():
            ...
            stat_df.index = flare_labels
            print(stat_label, stat_df)
            stat_df.to_csv(f"statistics/{is_coincident}/{time_combo}/{time_combo}_{stat_label}_{is_coincident}.csv")
