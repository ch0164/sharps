...
def main():
    ...
    labels = ["BC", "MX", "BX", "CM"]
    b_df = info_to_data(info_df.loc[info_df["xray_class"] == "B"], data_df)
    c_df = info_to_data(info_df.loc[info_df["xray_class"] == "C"], data_df)
    m_df = info_to_data(info_df.loc[info_df["xray_class"] == "M"], data_df)
    x_df = info_to_data(info_df.loc[info_df["xray_class"] == "X"], data_df)
    flare_dataframes = [
        pd.concat([b_df, c_df]),
        pd.concat([m_df, x_df]),
        pd.concat([b_df, x_df]),
    ...
    # for label, flare_df in zip(labels, flare_dataframes):
    #     print(label)
    #     n = 6
    ...
    for label, data_df in zip(labels, flare_dataframes):
        ...
        fig = px.scatter_3d(df, x="PC1", y="PC2", z="PC3", color="xray_class",
                            title=f"PCA ({label})\nTotal Explained Variance: {ev}")
        fig.write_html(f"pca/all_flares/{label}.html")
